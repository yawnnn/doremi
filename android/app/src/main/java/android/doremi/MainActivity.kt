...
class DoremiView(lst: List<Note>) {
    ...
    fun updateView(context: Context, note: Note) {
        val idx = this.lst.indexOfFirst { it.id == note.id }
        when (idx) {
            -1 -> this.lst.add(note)
            else -> this.lst[idx] = note
        }
        this.lst.sortedBy { it.ctime }
    }
}

data class DoremiEdit(val note: Note? = null, val startingBody: String = "")

class Doremi(context: Context) :ViewModel() {
    var state by mutableStateOf(DoremiState.VIEW)
    var view: DoremiView? = DoremiView(readNotes(context))  // when <viewState> == LIST
    var edit: DoremiEdit? = null                                 // when <viewState> == EDIT
    ...
    companion object {
        fun readNotes(context: Context): List<Note> {
            ...
            return dir.listFiles()?.filter { it.extension == "txt" }?.map {
                val note = Note.deserialize(it.readLines())
                assert(note.id == it.nameWithoutExtension)
                note
            } ?: emptyList()
        }
        ...
        setContent {
            DoremiTheme {
                        ...
                        when (intent.type) {
                            "text/plain" -> {
                                intent.getStringExtra(Intent.EXTRA_TEXT)?.let {
                                    doremi.setState(DoremiState.EDIT, edit = DoremiEdit(startingBody = it))
                                }
                            }
                ...
                when (doremi.state) {
                    ...
                    DoremiState.EDIT -> EditNote(doremi, onSave = { name, tags, body ->
                        val note = doremi.edit!!.note

                        if (note == null) {
                            Doremi.newNote(this, name, tags, body)
                        }
                        else {
                            val update = Note(id = note.id, name = name, tags = tags, body= body, ctime = note.ctime)
...
fun ViewNotes(doremi: Doremi) {
    val view = doremi.view!!
    val filters = remember(view.flt) { view.parseFilters(view.flt) }
    val notes = remember(view.lst, filters) {
        view.lst.filter { it.matches(filters) }
    }
...
@Composable
fun EditNote(doremi: Doremi, onSave: (String, List<String>, String) -> Unit, onCancel: () -> Unit) {
    val note = doremi.edit!!.note ?: Note()
